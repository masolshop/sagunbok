 ...
 * 버전 7.2.1 - doPost 함수 추가 (POST 요청 지원)
 * 
 * 시트 구조:
 ...
 * - 기업회원 시트에 사업자번호(A) 추가
 * - 기업회원 시트에 대표자명(C) 추가
 * - 기업회원 시트에 직함(E) 추가 (대표/재무담당자 등)
 * - 필드 순서 재정렬: 사업자번호/회사명/대표자명/기업회원분류/직함/이름/전화번호/이메일/비밀번호/가입일/승인여부/추천인
 */
...
function loginCompany(phone, password) {
  ...
  for (var i = 1; i < data.length; i++) {
    // G열: 전화번호, I열: 비밀번호
    if (isSamePhone(data[i][6], phone) && String(data[i][8]) === String(password)) {
      ...
      return {
        success: true,
        userData: {
          businessNumber: data[i][0] || '',  // A: 사업자번호
          companyName: data[i][1],           // B: 회사명
          ceoName: data[i][2] || '',         // C: 대표자명
          companyType: data[i][3],           // D: 기업회원분류
          position: data[i][4] || '',        // E: 직함
          name: data[i][5],                  // F: 이름
          phone: normalizedPhone.replace(/'/g, ''),
          email: data[i][7],                 // H: 이메일
          userType: 'company'
        }
      };
    }
