/**
 * 사근복 AI - Google Apps Script 백엔드 v2.8 (디버그 버전)
 * 
 * 필요한 Google Sheets:
...
function doGet(e) {
  const response = {
    ...
    version: '2.8-DEBUG',
    features: ['로그기록', '승인여부', '추천인검증', '컨설턴트비밀번호저장', 'CORS지원', '비밀번호디버그'],
...
function registerCompany(data) {
  try {
    ...
    const newRow = [
      ...
      data.phone,            // E: 전화번호
      data.email,            // F: 이메일
      String(data.password), // G: 비밀번호 (문자열로 변환)
      now,                   // H: 가입일
      '대기중',              // I: 승인여부
      `[${timestamp}] 회원가입 완료 (비밀번호 길이: ${String(data.password).length})` // J: 로그기록
    ];
...
function loginCompany(data) {
  try {
    ...
    for (let i = 1; i < sheetData.length; i++) {
      ...
      const storedPassword = String(sheetData[i][6]);   // G: 비밀번호 (문자열로 변환)
      const inputPassword = String(data.password);      // 입력 비밀번호 (문자열로 변환)
      const approvalStatus = sheetData[i][8]; // I: 승인여부
      
      if (phone === data.phone) {
        // 디버그 로그 기록
        const debugInfo = `입력: [${inputPassword}] (길이: ${inputPassword.length}, 타입: ${typeof data.password}), 저장: [${storedPassword}] (길이: ${storedPassword.length})`;
        addLog(sheet, i + 1, 10, '로그인 시도 - ' + debugInfo);
        ...
        if (trimmedStored === trimmedInput) {
          ...
          // 로그 기록
          addLog(sheet, i + 1, 10, '로그인 실패 - 비밀번호 불일치');
          return { 
            success: false, 
            ...
            debug: {
              ...
              storedTrimmed: trimmedStored,
              match: trimmedStored === trimmedInput
            }
          };
        }
      }
...
function loginConsultant(data) {
  try {
    ...
    for (let i = 1; i < sheetData.length; i++) {
      ...
      const storedPassword = String(sheetData[i][6]);   // G: 비밀번호 (문자열로 변환)
      const inputPassword = String(data.password);      // 입력 비밀번호
      const approvalStatus = sheetData[i][8]; // I: 승인여부
      
      if (phone === data.phone) {
        // 디버그 로그 기록
        const debugInfo = `입력: [${inputPassword}] (길이: ${inputPassword.length}), 저장: [${storedPassword}] (길이: ${storedPassword.length})`;
        addLog(sheet, i + 1, 10, '로그인 시도 - ' + debugInfo);
