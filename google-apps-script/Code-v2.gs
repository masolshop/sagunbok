/**
 * 사근복 AI - Google Apps Script 백엔드 (신규 버전 v2)
 * 
 * 필요한 Google Sheets:
 * 1. 기업회원 시트: 회사명, 기업회원분류, 추천인, 이름, 전화번호, 이메일, 비밀번호, 가입일, 승인여부, 로그기록
 * 2. 사근복컨설턴트 시트: 이름, 전화번호, 이메일, 직함, 소속사업단, 소속지사, 가입일, 승인여부, 로그기록
 */
 ...
 * 로그 기록 추가 함수
 */
function addLog(sheet, rowIndex, message) {
  try {
    const currentLog = sheet.getRange(rowIndex, 10).getValue(); // 로그기록 열 (J열)
    const timestamp = Utilities.formatDate(new Date(), 'Asia/Seoul', 'yyyy-MM-dd HH:mm:ss');
    const newLog = `[${timestamp}] ${message}`;
    const updatedLog = currentLog ? `${currentLog}\n${newLog}` : newLog;
    sheet.getRange(rowIndex, 10).setValue(updatedLog);
  } catch (error) {
...
function registerCompany(data) {
  try {
    ...
    for (let i = 1; i < consultantData.length; i++) {
      ...
      const approvalStatus = consultantData[i][7]; // 승인여부 열 (H열)
    ...
    for (let i = 1; i < sheetData.length; i++) {
      ...
      if (phone === data.phone) {
        ...
        if (password === data.password) {
          ...
          // 로그 기록
          addLog(sheet, i + 1, '로그인 실패 - 비밀번호 오류');
...
function loginConsultant(data) {
  try {
    ...
    for (let i = 1; i < sheetData.length; i++) {
      ...
      const approvalStatus = sheetData[i][7]; // H: 승인여부
  ...
  try {
    ...
    for (let i = 1; i < companyData.length; i++) {
      if (companyData[i][3] === data.name && companyData[i][5] === data.email) {
        // 로그 기록
        addLog(companySheet, i + 1, 'ID 찾기 요청');
  ...
  try {
    ...
    for (let i = 1; i < companyData.length; i++) {
      if (companyData[i][4] === data.phone && companyData[i][5] === data.email) {
        // 로그 기록
        addLog(companySheet, i + 1, '비밀번호 찾기 요청');
