 ...
 * 
 * 주요 변경 (v7.1.2):
 * - 비밀번호 비교 시 String() 변환 추가 (타입 불일치 방지)
 * - 시트의 숫자형 비밀번호(12345)와 문자열 비밀번호("12345") 모두 지원
 */
...
function loginCompany(phone, password) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (isSamePhone(data[i][4], phone) && String(data[i][6]) === String(password)) {
      ...
      writeLog('loginCompany', 'company', phone, 'SUCCESS', '로그인 성공: ' + data[i][0]);
