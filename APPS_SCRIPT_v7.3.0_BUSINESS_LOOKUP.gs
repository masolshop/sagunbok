 ...
 * 버전 7.3.0 - 국세청 사업자등록번호 조회 API 추가
 * 
 * 시트 구조:
 ...
 * 
 * 주요 변경 (v7.3.0):
 * - 🆕 국세청 사업자등록번호 조회 API 연동
 * - lookupBusinessNumber 함수 추가
 * - /api/external-data/lookup-business-number 엔드포인트 추가
 * - 국세청 API 키: c1ae465883fe093d1392e88bbd13b0cd998bba3ee21fae4322b1b6e394405bed
 * 
 * 이전 변경 (v7.2.3):
...
var CONFIG = {
  ...
  NTS_API_KEY: 'c1ae465883fe093d1392e88bbd13b0cd998bba3ee21fae4322b1b6e394405bed' // 국세청 API 키
};
 ...
 * @return {object} { success, companyName, ceoName, status, message }
 */
function lookupBusinessNumber(businessNumber) {
  ...
  try {
    // 사업자번호 정규화 (하이픈 제거)
    var cleanNumber = String(businessNumber).replace(/[-\s]/g, '');
    
    // 10자리 검증
    if (cleanNumber.length !== 10 || !/^\d{10}$/.test(cleanNumber)) {
      writeLog('lookupBusinessNumber', 'system', '', 'FAIL', '잘못된 사업자번호 형식: ' + businessNumber);
      return {
        success: false,
    ...
    // 국세청 API 호출
    var apiUrl = 'https://api.odcloud.kr/api/nts-businessman/v1/status';
    var params = {
    ...
    if (responseCode !== 200) {
      ...
      return {
        success: false,
        message: '국세청 API 호출에 실패했습니다. (상태 코드: ' + responseCode + ')'
      };
    }
    ...
    if (!result.data || !result.data[0]) {
      writeLog('lookupBusinessNumber', 'system', '', 'FAIL', '사업자번호를 찾을 수 없음');
      return {
        success: false,
    ...
    var statusName = businessData.b_stt;
    
    writeLog('lookupBusinessNumber', 'system', '', 'SUCCESS', 
      '조회 성공: ' + cleanNumber + ' (상태: ' + statusName + ')');
    
    // 회사명은 API에서 제공하지 않으므로 사업자번호만 검증
    return {
      success: true,
      ...
      message: '사업자번호 조회 성공',
      // 주의: 국세청 API는 회사명/대표자명을 제공하지 않음
      companyName: '', 
      ceoName: ''
    };
    
  } catch (error) {
    Logger.log('사업자번호 조회 오류: ' + error.toString());
    writeLog('lookupBusinessNumber', 'system', '', 'ERROR', '예외 발생: ' + error.toString());
    
    return {
      success: false,
      message: '사업자번호 조회 중 오류가 발생했습니다: ' + error.toString()
    };
  }
}
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (isSamePhone(rowPhone, phone)) {
      if (String(rowPassword).trim() === String(password).trim()) {
        ...
        writeLog('loginCompany', 'company', phone, 'SUCCESS', '로그인 성공: ' + data[i][1]);
        
        return {
          success: true,
          userData: {
            businessNumber: data[i][0] || '',
            companyName: data[i][1],
            ceoName: data[i][2] || '',
            companyType: data[i][3],
            position: data[i][4] || '',
            name: data[i][5],
...
function approveMember(userType, phone) {
  ...
  if (userType === 'company') {
    sheetName = '기업회원';
    phoneColumn = 7;      // G열 (전화번호)
    approvalColumn = 11;  // K열 (승인여부)
    emailColumn = 8;      // H열 (이메일)
    nameColumn = 6;       // F열 (이름)
  ...
  for (var i = 1; i < data.length; i++) {
    if (isSamePhone(data[i][phoneColumn - 1], phone)) {
      ...
      writeLog('approveMember', userType, phone, 'SUCCESS', '회원 승인 완료');
...
function rejectMember(userType, phone, reason) {
  writeLog('rejectMember', userType, phone, 'START', '회원 반려 시도');
  ...
  for (var i = 1; i < data.length; i++) {
    if (isSamePhone(data[i][phoneColumn - 1], phone)) {
      ...
      writeLog('rejectMember', userType, phone, 'SUCCESS', '회원 반려 완료');
...
function getAllMembers() {
  ...
  for (var i = 1; i < companyData.length; i++) {
    members.push({
      userType: 'company',
      name: companyData[i][5],           // F: 이름
      phone: companyData[i][6],          // G: 전화번호
      email: companyData[i][7],          // H: 이메일
      companyName: companyData[i][1],    // B: 회사명
      referrer: companyData[i][11],      // L: 추천인
      approvalStatus: companyData[i][10] || '승인대기', // K: 승인여부
      registeredAt: companyData[i][9]    // J: 가입일
    });
  }
  ...
  for (var i = 1; i < managerData.length; i++) {
    members.push({
      ...
      email: managerData[i][2],
      region: managerData[i][4],
      referrer: managerData[i][9] || '',
      approvalStatus: managerData[i][8] || '승인대기',
      registeredAt: managerData[i][7]
    });
  }
  ...
  for (var i = 1; i < consultantData.length; i++) {
    members.push({
      ...
      region: consultantData[i][4],
      referrer: consultantData[i][9] || '',
      approvalStatus: consultantData[i][8] || '승인대기',
      registeredAt: consultantData[i][7]
    });
  }
