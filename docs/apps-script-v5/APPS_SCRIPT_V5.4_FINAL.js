 ...
 * - loginConsultant 함수가 I열 승인상태를 읽고 반환
 * - registerConsultant 함수가 I열 승인상태를 '승인전표'로 저장
 * - 기업회원과 컨설턴트 모두 I열에 승인상태 저장
 * 
 * 🎯 V5.4.1 변경사항:
 * - doPost에서 JSON body 파싱 지원 추가
 * - 프록시 서버에서 보내는 { action, data } 형식 지원
 * - H열: 추천인
 * - I열: 승인상태
 * 
 * 🎯 핵심 개선사항:
 * 1. ✅ 시트 구조를 회원가입 순서에 맞춰 재작성
 * 2. ✅ 추천인 검증 절대 강화 (사근복컨설턴트 매칭 필수)
 * 3. ✅ 전화번호 앞자리 0 손실 해결 ('010-1234-5678 형식)
 * 4. ✅ 기업유형, 추천인 정확히 저장
 * 5. ✅ 상세 로깅 추가
 * 
 * ============================================================
 ...
 *   D: 이름
 *   E: 핸드폰번호 (010-1234-5678 형식) ← ⚠️ 일반 텍스트로 설정!
 *   F: 이메일
 *   G: 비밀번호
 *   H: 추천인 ← 변경됨!
 *   I: 승인상태 (승인전표/승인완료) ← 변경됨!
 *   J: (비어있음)
 *   K: 마지막로그인
 * 
 * [사근복컨설턴트] 시트:
...
function loginCompany(phone, password) {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const approvalStatus = String(row[8]);      // I열: 승인상태 (변경됨!)
      ...
      if (storedPhone === normalizedPhone) {
        ...
        return {
          ...
          userData: {
            ...
            email: row[5],          // F열: 이메일
            referrer: row[7],       // H열: 추천인 (변경됨!)
            approvalStatus: row[8], // I열: 승인상태 (변경됨!)
      ...
      if (storedPhone === normalizedPhone) {
        ...
        return {
          ...
          userData: {
            ...
            branch: row[6],
            registeredAt: row[7],
            approvalStatus: row[8] || '승인전표'  // I: 승인상태
          }
        };
      }
    }
...
function doPost(e) {
  try {
    ...
    if (e.postData && e.postData.contents) {
      ...
      Logger.log('doPost 요청 (JSON body): ' + action);
      Logger.log('데이터: ' + JSON.stringify(params));
    } else {
      // Form 요청 처리 (레거시)
      params = e.parameter;
      action = params.action;
      Logger.log('doPost 요청 (form): ' + action);
...
function runAllTests() {
  Logger.log('🧪 Sagunbok Apps Script V5.4 자동 테스트 시작');
  ...
  if (failCount === 0) {
    Logger.log('\n🎉 모든 테스트 통과! Apps Script V5.4가 정상 작동합니다.');
  } else {
...
function validateSheetStructure() {
  Logger.log('📊 Google Sheets 구조 검증 (V5.4)');
  Logger.log('='.repeat(50));
