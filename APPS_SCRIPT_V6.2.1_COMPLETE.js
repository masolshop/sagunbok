 ...
 * 버전 6.2.1 - 로그인 버그 수정 (2026-01-24)
 * 
 * 주요 변경사항 (v6.2.1):
 * - loginCompany 함수의 normalizedPhone, existingPhone 변수 버그 수정
 * - 로그인 로직 안정화
 * - 에러 메시지 개선
 * 
 * 기존 기능 (v6.2):
...
function loginCompany(phone, password) {
  try {
    ...
    // ✅ v6.2.1 수정: normalizedPhone 변수 정의
        ...
        if (storedPassword === password) {
          ...
          return {
            ...
            user: {
              ...
              referrer: String(row[2]),
              name: String(row[3]),
              phone: storedPhone, // ✅ v6.2.1 수정: existingPhone → storedPhone
              email: String(row[5])
            }
          };
        } else {
...
function doGet(e) {
  ...
  return ContentService
    .createTextOutput(JSON.stringify({
      ...
      message: '사근복 AI Apps Script v6.2.1 - 로그인 버그 수정 완료'
    }))
